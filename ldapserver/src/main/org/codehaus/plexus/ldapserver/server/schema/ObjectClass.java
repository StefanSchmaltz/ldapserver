  ...
  public String toString()
  {
    ...
    if ( getMust() != null && !getMust().isEmpty() )
    {
      if ( getMust().size() > 1 )
      {
        ...
        Enumeration e = getMust().elements();
        while (e.hasMoreElements())
        {
          objClass = objClass.concat(((DirectoryString) e.nextElement()).toString());
    ...
    if ( getMay() != null && !getMay().isEmpty() )
    {
      ...
      {
        objClass = objClass.concat( " MAY " + getMay().elementAt( 0 ) );
      }
    }
    objClass = objClass.concat( " )" );
    return objClass;
